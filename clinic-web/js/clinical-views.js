...
function renderDoctorDashboard() {
  ...
  return `
    ${pageHeader('Phòng 201 / Nội khoa', 'TS.BS Trần Thị Minh', date,
    ...
    <div class="content-grid">
      ...
      <section class="surface" aria-labelledby="clinicalActivityHeading">
        ...
        <div class="stack">
          ...
          <div class="actions">
            ${button('Mở phiếu khám tiếp theo', 'callNextPatient()', 'arrow-right', 'secondary')}
          </div>
        </div>
      </section>
    </div>
...
function renderClinicalProgress(label, value, percent) {
  return `
    <div class="stack">
      ...
      <progress class="clinical-progress" max="100" value="${Number(percent)}" aria-label="${escapeHtml(label)}" aria-valuetext="${escapeHtml(value)}">${Number(percent)}%</progress>
    </div>
  `;
}
...
function renderDoctorExamination() {
  ...
  const actions = button('Hàng đợi', "navigate('doctor-dashboard', 'doctor')", 'arrow-left', 'ghost') + (
    patient.status !== 'in_progress'
      ? button('Bắt đầu khám', `startPatientExamination(${Number(patient.id)})`, 'play', 'primary', `id="btnStartExam" ${disabled}`)
      : button('Lưu nháp', `triggerManualSaveDraft(${Number(patient.id)})`, 'save', 'secondary', disabled)
  );
  ...
  return `
    ...
    <div class="section-heading wrap">
      <div class="row wrap">
        ...
        <span id="examStatusBadge" class="badge badge-${patient.isLocked ? 'success' : patient.status === 'in_progress' ? 'info' : 'warning'}">
...
function renderClinicalPatientSummary(patient) {
  const names = String(patient.name || '').trim().split(/\s+/);
      ...
      <div class="section-heading wrap">
        <div class="row wrap">
          ...
          <div>
            ...
            <p class="muted small">Ngày sinh: ${escapeHtml(formatDate(patient.dob))} / ${escapeHtml(patient.gender)} / ${escapeHtml(patient.phone)}</p>
          </div>
        </div>
        <span class="badge badge-info">STT #${escapeHtml(String(patient.stt || 1).padStart(2, '0'))}</span>
      </div>
      <div class="row wrap">
        <span class="badge badge-${patient.allergy !== 'Không' ? 'danger' : 'success'}">Dị ứng: ${escapeHtml(patient.allergy)}</span>
...
function renderClinicalField({ id, label, type = 'text', value, handler = '', placeholder, unit, locked }) {
...
function renderClinicalVitals(patient, fieldValue) {
  const fields = [
    ...
    { id: 'vitalHeight', label: 'Chiều cao', value: '170', placeholder: '170', unit: 'cm', type: 'number' },
    { id: 'vitalWeight', label: 'Cân nặng', value: '65', placeholder: '65', unit: 'kg', type: 'number' },
  ...
  return `
    <section class="clinical-section" aria-labelledby="clinicalVitalsHeading">
      ...
      <div class="vitals-grid">
        ${fields.map(field => renderClinicalField({
          ...field,
...
function renderClinicalFindings(patient, fieldValue) {
  ...
  const fields = [
    ...
    { id: 'examHistory', label: 'Bệnh sử hiện tại & Diễn tiến', type: 'textarea', placeholder: 'Nhập bệnh sử hiện tại...', value: 'Khởi phát cách đây 3 ngày với cảm giác ợ chua, cồn cào vùng thượng vị sau ăn. Đã tự dùng thuốc dạ dày không đỡ.' },
    { id: 'examSymptoms', label: 'Ghi nhận triệu chứng lâm sàng', type: 'textarea', value: 'Bụng mềm, ấn đau tức nhẹ thượng vị, không đề kháng. Tim đều, phổi trong, không ran.' },
    { id: 'examPrelimDiagnosis', label: 'Chẩn đoán sơ bộ ban đầu', value: 'Theo dõi Viêm loét dạ dày tá tràng cấp' }
  ];
  const diagnoses = [
    ['K29', 'K29 - Viêm dạ dày'],
  ...
  return `
    <section class="clinical-section" aria-labelledby="clinicalFindingsHeading">
      <div class="section-heading"><h2 id="clinicalFindingsHeading">Khám lâm sàng &amp; chẩn đoán</h2></div>
      <div class="stack">
        ${fields.map(field => renderClinicalField({
          ...field, value: fieldValue(field.id, field.value), handler, locked: patient.isLocked
        })).join('')}
        <div class="form-grid">
          ${renderClinicalField({
            id: 'examDiagnosis', label: 'Chẩn đoán xác định',
            value: fieldValue('examDiagnosis', 'Viêm dạ dày tá tràng / Theo dõi trào ngược dạ dày thực quản'),
            handler, locked: patient.isLocked
          })}
          <div class="field">
            <label for="examIcd10">Mã ICD-10</label>
            <select class="form-input" id="examIcd10" onchange="${escapeHtml(handler)}" ${patient.isLocked ? 'disabled' : ''}>
...
function renderClinicalFollowUp(patient, fieldValue) {
  return `
    <section class="clinical-section" aria-labelledby="clinicalFollowUpHeading">
      <div class="section-heading"><h2 id="clinicalFollowUpHeading">Lời dặn &amp; hẹn tái khám</h2></div>
      <div class="stack">
        ${renderClinicalField({
          id: 'examNotes', label: 'Chế độ dinh dưỡng & sinh hoạt', type: 'textarea',
          value: fieldValue('examNotes', 'Ăn uống điều độ, tránh thức ăn cay nóng, kiêng rượu bia và cà phê. Nghỉ ngơi hợp lý.'),
          locked: patient.isLocked
        })}
        ${renderClinicalField({
          id: 'examFollowUp', label: 'Hẹn ngày tái khám', type: 'date',
          value: fieldValue('examFollowUp', '2026-09-28'), locked: patient.isLocked
        })}
      </div>
    </section>
  `;
...
function renderClinicalRemoveButton(label, handler, locked) {
  return `<button type="button" class="icon-btn" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}" onclick="${escapeHtml(handler)}" ${locked ? 'disabled' : ''}>${icon('trash-2')}</button>`;
}
...
function renderClinicalOrders(patient, totalCls) {
  return `
    <section class="clinical-section" aria-labelledby="clinicalOrdersHeading">
      ...
      <div class="summary-line"><span>Tổng chi phí cận lâm sàng</span><strong>${escapeHtml(money(totalCls))}</strong></div>
    </section>
  `;
}
...
function renderClinicalOrder(order, index, locked) {
  const status = order.status === 'COMPLETED'
    ...
    : order.status === 'PAID' ? badge('Đã thu tiền', 'info') : badge('Chờ thu tiền', 'warning');
  const disabled = locked ? 'disabled' : '';
  const action = order.status === 'ORDERED'
    ? button('Thu tiền tại quầy', `payClsOrder(${index})`, 'credit-card', 'secondary', disabled)
    : order.status === 'PAID'
      ? button('KTV nhập kết quả', `enterClsResult(${index})`, 'file-pen-line', 'primary', disabled)
      : button('Cập nhật kết quả KTV', `enterClsResult(${index})`, 'file-pen-line', 'ghost', disabled);
  return `
    <article class="order-item stack" aria-labelledby="clinicalOrderHeading${index}">
      ...
      <div class="summary-line"><span>Chi phí dịch vụ</span><strong>${escapeHtml(money(order.price))}</strong></div>
  ...
  return `
    <div class="stack">
      ...
      ${order.indicators && order.indicators.length > 0 ? `
        <div class="table-wrap" role="region" aria-labelledby="clinicalOrderHeading${index}" tabindex="0">
          <table class="data-table results-table">
            ...
            <tbody>
              ${order.indicators.map(indicator => `
                <tr>
      ...
      <div class="actions wrap">
        ${order.resultFileUrl ? renderClinicalResultLink(order.resultFileUrl, 'Phiếu kết quả PDF', 'file-text') : ''}
...
function renderClinicalResultLink(url, label, iconName) {
  ...
  if (!href || href === '#') return `<span class="muted small">${escapeHtml(label)}: liên kết không khả dụng</span>`;
  return `<a class="btn btn-ghost small" href="${href}" target="_blank" rel="noopener noreferrer">${icon(iconName)}${escapeHtml(label)}</a>`;
}
...
function renderClinicalPrescriptions(patient, totalMeds) {
  ...
  return `
    <section class="clinical-section" aria-labelledby="clinicalPrescriptionHeading">
      ...
      <div class="summary-line"><span>Tổng tiền thuốc</span><strong>${escapeHtml(money(totalMeds))}</strong></div>
    </section>
  `;
}
  ...
  return `
    <section class="surface" aria-labelledby="clinicalCompletionHeading">
      ...
      <dl class="summary-list">
        <div class="summary-line"><dt>Khám chuyên khoa</dt><dd>${escapeHtml(money(200000))}</dd></div>
        <div class="summary-line"><dt>Cận lâm sàng</dt><dd>${escapeHtml(money(totalCls))}</dd></div>
        <div class="summary-line"><dt>Thuốc theo đơn</dt><dd>${escapeHtml(money(totalMeds))}</dd></div>
        <div class="summary-line"><dt><strong>Tổng viện phí</strong></dt><dd><strong>${escapeHtml(money(200000 + totalCls + totalMeds))}</strong></dd></div>
      </dl>
      ${patient.isLocked ? `
